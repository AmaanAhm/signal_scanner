...
// ── Per-stock backtest ──
// Step 1: Selected TF generates BUY signal via Lorentzian
// Step 2: Fetch finer TF (5m) data for precise retest/exit detection
// Step 3: Entry on first 5m bar where price touches signal price
// Step 4: Exit on +N% target or -M% stoploss (walking through 5m bars)
...
export const backtestStock = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ entries: BacktestEntry[]; error?: string }> => {
    try {
      ...
      // ── Step 3: Fetch finer TF data for retest/exit ──
      const retestTf = RETEST_TF[tf] || "5m";
      const retestMaxDays = RETEST_MAX_DAYS[retestTf] || 55;
      const retestFetchStart = new Date(Math.max(
        period1.getTime() - 86400_000,
        Date.now() - retestMaxDays * 86400_000,
      ));

      let retestBars: Bar[];
      try {
        const retestResult = await yf.chart(data.symbol, {
          period1: retestFetchStart,
          interval: retestTf as any,
        }, { validateResult: false }) as any;

        retestBars = [];
        for (const q of retestResult.quotes) {
          if (q.open == null || q.high == null || q.low == null || q.close == null) continue;
          const time = q.date instanceof Date ? q.date.getTime() : new Date(q.date as string).getTime();
      ...
      for (let i = 0; i < retestBars.length; i++) {
        const bar = retestBars[i];

        // Activate any signals that have been generated by this bar's time
        while (sigIdx < signals.length && signals[sigIdx].time <= bar.time) {
          pending = signals[sigIdx];
          sigIdx++;
        }

        // Check retest of pending signal
        if (pending && bar.time > pending.time && bar.time >= rangeStart) {
          ...
          if (touching) {
            ...
            if (hitEntry) {
              ...
              // Walk forward through ALL subsequent bars
              let found = false;
              for (let k = i + 1; k < retestBars.length; k++) {
                ...
                if (hitFwd) {
                  ...
                  exitTime = fTime;
                  exitDate = fDate;
                  holdingMinutes = Math.round((fBar.time - bar.time) / 60000);
                  found = true;
                  break;
                }
              }
